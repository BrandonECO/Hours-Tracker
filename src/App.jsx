...
export default function HoursTracker() {
  ...
  const [hideDollarAmounts, setHideDollarAmounts] = useState(() => loadString("hoursTracker.hideDollarAmounts", "false") === "true");
  ...
  useEffect(() => saveString("hoursTracker.hideDollarAmounts", hideDollarAmounts ? "true" : "false"), [hideDollarAmounts]);
  ...
  function importBackupJson(event) {
    ...
    reader.onload = () => {
      try {
        ...
        if (typeof backup.hideDollarAmounts !== "undefined") setHideDollarAmounts(Boolean(backup.hideDollarAmounts));
      } catch {
  ...
  function clearAllData() {
    setEntries([]);
    setPhysicalClocked("");
    setExtraHours(EMPTY_EXTRA_HOURS);
  }
  ...
  return (
    <div className={`${theme === "dark" ? "bg-zinc-950 text-zinc-100" : "light-mode bg-slate-100 text-slate-950"} min-h-screen p-4 md:p-8`}>
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <KpiGrid totals={totals} rates={rates} hideDollarAmounts={hideDollarAmounts} />
        <PerformanceOverview totals={totals} goal={goal} />
        <TechnicianInsights totals={totals} rates={rates} goal={goal} daysLeft={daysLeft} hideDollarAmounts={hideDollarAmounts} />
        ...
        <SettingsModal profile={profile} setProfile={setProfile} period={period} setPeriod={setPeriod} rates={rates} setRates={setRates} theme={theme} setTheme={setTheme} hideDollarAmounts={hideDollarAmounts} setHideDollarAmounts={setHideDollarAmounts} open={showSettings} onClose={() => setShowSettings(false)} />
        <AddEntry form={form} setForm={setForm} addEntry={addEntry} exportCsv={exportCsv} exportPdf={() => window.print()} exportBackupJson={exportBackupJson} importBackupJson={importBackupJson} clearAllEntries={clearAllData} />
...
function Header({ profile, period, status, onOpenSettings }) {
  return (
    <header className="relative flex flex-col items-center gap-4 text-center">
      <div className="absolute right-0 top-0 flex items-center gap-3"><span className={`w-fit rounded-full px-4 py-2 text-sm font-semibold ${status.cls}`}>{status.label}</span><button onClick={onOpenSettings} className="flex h-11 w-11 items-center justify-center rounded-2xl border border-zinc-700 bg-zinc-800 text-zinc-100 shadow-lg transition hover:bg-zinc-700" aria-label="Open settings"><Settings2 className="h-5 w-5" /></button></div>
      <div><h1 className="text-4xl font-black tracking-[0.35em] uppercase">HOURS TRACKER</h1><p className="text-zinc-400">Flagged vs actual vs physically clocked{profile.technician ? ` — ${profile.technician}` : ""}{profile.role ? `, ${profile.role}` : ""}</p>{profile.dealership && <p className="mt-1 text-sm text-zinc-500">{profile.dealership}</p>}<p className="mt-1 text-sm text-zinc-500">Period: {period.start ? formatDisplayDate(period.start) : "Not set"} to {period.end ? formatDisplayDate(period.end) : "Not set"}</p></div>
    </header>
  );
}

function KpiGrid({ totals, rates, hideDollarAmounts }) {
...
function TechnicianInsights({ totals, rates, goal, daysLeft, hideDollarAmounts }) {
